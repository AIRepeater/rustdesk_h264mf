...
pub fn check_available_hwcodec() -> String {
    ...
    // The external hwcodec crate's encoder probe loop may not detect h264_mf
    // on all GPUs (e.g. Moore Threads MTT S70 whose MFT is registered via
    // Media Foundation). Inject it directly as a fallback so that the
    // upstream vendor SDK paths (qsv/nvenc/amf) still take priority.
    #[cfg(windows)]
